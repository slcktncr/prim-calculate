...
import { AlertTriangle, RefreshCw, Calendar, Undo2 } from 'lucide-react';
  ...
  const handleRestoreSale = async (saleId) => {
    if (window.confirm('Bu satışın iptalini geri almak istediğinizden emin misiniz?')) {
      try {
        ...
        console.error('İptal geri alma hatası:', error);
        toast.error(error.response?.data?.message || 'İptal geri alma işlemi başarısız');
      }
    }
  };
